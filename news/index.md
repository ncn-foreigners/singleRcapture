...
- The `logLik.singleRStaticCountData` method now has a `type` argument
  ...
  log-likelihood function (by default) or if `deriv` argument is set to
  `1` or `2` respectively either a gradient or hessian of log-likelihood
  function.
...
- Introduced `singleRStaticCountData` sub class for `singleRclass` and
  ...
  `countreg::zerotrunc` or
  [`VGAM::vglm`](https://rdrr.io/pkg/VGAM/man/vglm.html)/[`VGAM::vgam`](https://rdrr.io/pkg/VGAM/man/vgam.html)
...
- Added deviance for all negative binomial based models. (NOTE: They are
  very slow for now and I believe it may change after I verify one
...
- features and improvements:
  ...
  - Added `predict.singleR` method which offers standard error for both
    `link`, `response` as well as `mean` predictions
  ...
  - Added information about
    [`stats::optim`](https://rdrr.io/r/stats/optim.html) error codes
...
- features
  ...
  - Added more warnings
  - Added some more methods for `singleR` class in some commonly used
    `glm` functions, in particular `texreg::screenreg` should work well
    now
- changes
...
- features:
  ...
  - Added most of sandwich capabilities to the package, in particular:
    ...
    - `vcovCL` should work on `singleR` class objects should work with
      `"HC0"` and `"HC1"` `type` argument values
