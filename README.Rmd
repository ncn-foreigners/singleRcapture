...
- Zero-truncated one-inflated and one-inflated zero-truncated Poisson, geometric and negative binomial model.
- Zero-one-truncated Poisson, geometric and negative binomial models.
- Generalized Chao's and Zelterman's models based on logistic regression.
- Modified Chao's one-inflation-robust `oichao()` model based on counts 2 and 3.
- Standalone `ratioReg()` support for ratio regression with optional one-inflation model selection.
...
For more details we see the `singleRcapture: An R Package for Single-Source Capture-Recapture Models` vignette on [CRAN](https://cran.r-project.org/package=singleRcapture) or [pkgdown website](https://ncn-foreigners.ue.poznan.pl/singleRcapture/articles/singleRcapture.html).
...
The main function of this package is `estimatePopsize` which fits regression
on specified distribution and then uses fitted regression to estimate the
population size. 

Let's look at a model from 2003 publication (Van Der Heijden, P. G., Bustami, R., Cruyff, M. J., Engbersen, G., & Van Houwelingen, H. C. (2003). Point and interval estimation of the population size using the truncated Poisson regression model. Statistical Modelling, 3(4), 305-322.). The call to `estimatePopsize` will look very similar to anyone who used the 
`stats::glm` function:
...
The package also includes `oichao()`, a modified Chao family that uses the
counts `2` and `3` instead of `1` and `2`, which makes it more robust when
one-inflation is a concern:
...
ratio-regression approach. It is separate from `singleRmodels()` because it
works on neighbouring marginal-frequency ratios rather than on unit-level
count likelihoods:
...
The default ratio-regression plot shows the observed log-ratios together with
the fitted `M0` and `M1` curves:

```{r ratioReg-plot, fig.show="hold", out.width="75%"}
plot(ratioModel)
```
...
- `qq` - the normal quantile-quantile plot for pearson residuals (default),
...
User can also pass arguments to specify additional information such as plot 
...
```{r plot-example, fig.show="hold", out.width="75%"}
...
Lastly `singleRcapture` offers some post-hoc procedures for example a function 
`stratifyPopsize` that estimates sizes of user specified sub populations and
returns them in a `data.frame`:

```{r stratifyPopsize-example1}
...
the results are significantly different (the warning issued concerns the second derivative test for existence of local minimum, here it was inconclusive but we manually checked that fitting process
found the optimal regression coefficients it's here to provide more information
to the user):

```{r plot-inflated, fig.show="hold", out.width="75%"}
plot(modelInflated2, 
     plotType = "bootHist", 
     labels = TRUE, 
     ylim = c(0, 175),
...
Work on this package is supported by the National Science Centre through OPUS 20 grant no. 2020/39/B/HS4/00941.   

## Citation

Chlebicki, P., & Beręsewicz, M. (2026). singleRcapture: An R Package for Single-Source Capture-Recapture Models. Journal of Statistical Software, 115(1), 1–30. https://doi.org/10.18637/jss.v115.i01
