...
which will be covered in detail in Section \ref{sec-theory}. The package
...
parameters $\bbeta$ for $\mathbb{P}[Y_{k}>0|\bx_{k}]$ in
\eqref{eq:eq-ht-estimator}.
...
$I_{k}$'s and after some omitted simplifications, is optimally estimated
by:
...
\begin{itemize}
    ...
    \item Zero-truncated (`zt`$^\ast$) and zero-one-truncated (`zot`$^\ast$) Poisson \citep[cf. ][]{zotmodels}, geometric, NB type II (NB2) regression, where the non-truncated distribution is parameterized as:
    \begin{equation*}
    ...
    \end{equation*}
    \item Zero-truncated one-inflated (`ztoi`$^\ast$) modifications, where the count data variable $Y^{\ast}$ is defined such that its distribution statisfies:
    \begin{equation*}
    ...
    \end{equation*}
    \item One-inflated zero-truncated (`oizt`$^\ast$) modifications, where the count data variable $Y^{\ast}$ is defined as:
    \begin{equation*}
    ...
    \end{equation*}
    Note that `ztoi`$^\ast$ and `oizt`$^\ast$ distributions are equivalent, in the sense that the maximum value of the likelihood function is equal for both of those distributions given any data, as shown by \cite{bohning2023equivalence} but population size estimators are different.
\end{itemize}
...
algorithm is employed for this purpose as presented in Algorithm
\ref{algo-estimation}; full details are available in \cite{VGAM-main}.
...
\begin{algorithm}[ht!]
\small
\caption{The modified IRLS algorithm used in the **singleRcapture** package}
\label{algo-estimation}\DontPrintSemicolon
\nlset{1} Initialize with `iter`$\leftarrow 1, \boeta\leftarrow$`start`
...
\nlset{5} If convergence is achieved or `iter` is higher than `maxiter`, stop the algorithm, 
    else `iter`$\leftarrow 1+$`iter` and return to step 2.
\end{algorithm}
...
\cite{BoehningFriedl2021}) and nonparametric. The nonparametric version
...
\subsection[The estimatePopsize function]{The `estimatePopsize` function}
...
important arguments are given in Table \ref{tab-arguments-popsize}; the
obligatory ones are `formula, data, model`.
...
\hline
\end{tabular}
\caption{A description of `estimatePopsize` function arguments}
...
\subsection[Controlling variance estimation with controlPopVar]{Controlling variance estimation with `controlPopVar`}
...
\begin{itemize}
  \item `alpha`, `B` -- the significance level and the number of bootstrap samples to be performed, respectively, with $0.05$ and $500$ being the default options.
  \item `cores` -- the number of process cores to be used in bootstrap (1 by default); parallel computing is enabled by **doParallel** \citep{doParallel}, **foreach** \citep{foreach} and **parallel** packages \citep{parallel}.
  \item `keepbootStat` -- a logical value indicating whether to keep a vector of statistics produced by the bootstrap.
  \item `traceBootstrapSize`, `bootstrapVisualTrace` --  logical values indicating whether sample and population size should be tracked (`FALSE` by default); these work only when `cores` = 1.
    \item `fittingMethod`, `bootstrapFitcontrol` -- the fitting method (by default the same as the one used in the original call) and control parameters (`controlMethod`) for model fitting in the bootstrap.
\end{itemize}
...
\citep[cf.][]{rootogram}, for comparing fitted and marginal frequencies,
...
\subsection[The stratifyPopsize function]{The `stratifyPopsize` function}
...
We have provided integration with the **sandwich** \citep{sandwich}
...
\begin{itemize}
\item a formula with the empty left hand side, as shown in the example above (e.g. `~ gender * age`),
\item a logical vector with the number of entries equal to the number of rows in the dataset, in which case only one stratum will be created (e.g. `netherlandsimmigrant$gender == "male"`),
\item a vector of names of explanatory variables, which will result in every level of the explanatory variable having its own sub-population for each variable specified (e.g. `c("gender", "age")`),
...
\section[Classes and S3Methods]{Classes and `S3Methods`}\label{sec-methods}
...
\begin{itemize}
  \item `pointEstimate`, `variance` -- numerics containing the point estimate and variance of this estimate.
  \item `confidenceInterval` -- a `data.frame` with confidence intervals.
  \item `boot` -- If the bootstrap was performed a numeric vector containing the $\hat{N}$ values from the bootstrap, a character vector with value `"No bootstrap performed"` otherwise.
  \item `control` -- a `controlPopVar` object with controls used to obtain the object.
\end{itemize}
...
`model.matrix` & it has the possibility of returning the $X_{\text{vlm}}$ matrix defined in \ref{X_vlm-definition};\\
...
`AIC, BIC, extractAIC, family, confint, df.residual, model.frame, hatvalues, nobs, print, sigma, influence, rstudent, rstandard`  & it works exactly like `glm` counterparts.\\
\hline 
\end{tabular}
\caption{`S3Methods` implemented in the **singleRcapture**}
\label{tab-methods}
\end{table}
...
methods is presented in Table \ref{tab-methods}.
...
is exemplified in the Appendix \ref{sec-family}. An example of
implementing a custom family described in Appendix \ref{sec-custom} is
...
(e.g. via the **brms** package; \cite{carpenter2017stan, brms}) and for
one-inflation models we can use the recent approach proposed by
\cite{tuoto2022bayesian} and implement our own families using the
**brms** package.
...
from @chao-generalization with covariate dependency: \begin{align*}
  \log(\lambda) &=
...
A factory function for creating the following functions:
  ...
  \end{equation*}
  from the $\boldsymbol{y}$ vector and the $\bX_{vlm}$ matrix, which has the `deriv` argument with possible 
  values in `c(0, 1, 2)` that determine which derivative to return; the default value is `0`, which represents the minus log-likelihood; \\
