...
perpetrators_data <- data.frame(
  y = c(rep(1, 15169), rep(2, 1957), rep(3, 393), 
        rep(4, 99), rep(5, 28), rep(6, 16))
)
...
fit_synthetic <- estimatePopsize(y ~ 1, data = synthetic_data, 
                                 model = oichao(bias_corr = T), method = "IRLS")

# Hand calculation
expected_synthetic <- compute_oichao(f2 = 95, f3 = 32, n = 824, bias_corr = T)
